...
import { z } from 'zod';
import { endEspnDiscovery, openFantasyFootballInDiscovery } from '../auth/login-window';
...
const channels = ['discovery:enabled', 'discovery:snapshot', 'discovery:analysis', 'discovery:export', 'discovery:open-football', 'discovery:end'] as const;
export function registerDiscoveryIpc(): void {
  ...
  ipcMain.handle('discovery:open-football', (_event, input) => { EmptyRequest.parse(input); return openFantasyFootballInDiscovery(); });
